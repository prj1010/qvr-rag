...
uv pip install --python "%PY%" --upgrade "langchain-text-splitters>=0.3,<0.4" "markitdown[pdf,docx]>=0.1,<1" "docling-slim[format-pdf,feat-ocr-rapidocr-onnx]>=2.121,<3" "httpx>=0.27,<1" "sqlite-vec>=0.1.9,<0.2" "kokoro-onnx>=0.6.1,<0.7" "soundfile>=0.12,<1" "numpy>=1.26,<3"
if errorlevel 1 exit /b 1
